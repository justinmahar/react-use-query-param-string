...
`useQueryParamString(key: string, defaultValue: string, clearOnInitial?: boolean) => [value, setValue, initialized, clear]`
...
| clearOnDefault | `string` | Optional. When true, the query param is cleared from the URL when the current value equals the default value. Default true. |
...
Remove a query param key from the URL entirely. Resets to the default value.
